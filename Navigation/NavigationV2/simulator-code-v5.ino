...
{
  while(!((t < targetTheta + 2  && t > targetTheta - 2)))
  {
...
void travel(float distance)
{
  ...
  float inY = y;
  setBothMotors(125);
  while(!((x < (inX + distance + 0.05) && x > (inX + distance - 0.05)) || (y < (inY + distance + 0.05) && y > (inY + distance - 0.05)) || (y < (inY - distance + 0.05) && y > (inY - distance - 0.05))))
  {
     update();
  }
  Tank.turnOffMotors();
